...
const VALUES = [
  { icon: Heart, title: 'Made with Love', body: 'Every item is crafted by hand in our home kitchen. No factory lines, no shortcuts — just honest baking the way it should be done.' },
  { icon: Clock, title: 'Always Fresh', body: 'We bake to order. Nothing sits on a shelf. When your order arrives, it was made for you — hours ago, not days.' },
  { icon: Star, title: 'No Compromises', body: "We use real butter, pure cocoa, and fresh eggs. Our standards are simple: if we wouldn't eat it ourselves, we won't sell it." },
]
  ...
  return (
    <>
      <section className="bg-gradient-to-br from-[#F2E4D5] via-[#EDD8C2] to-[#E4C9AA] py-20 lg:py-28 px-4 sm:px-8 lg:px-20 text-center">
        ...
        <p className="text-base lg:text-lg text-muted max-w-[520px] mx-auto leading-[1.7]">
          A home bakery built on one belief — that homemade food, made with care, is always worth it.
        </p>
      </section>

      <section className="bg-beige">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center py-16 lg:py-28 px-4 sm:px-8 lg:px-20">
          <motion.div {...fadeUp(0)} className="h-[300px] sm:h-[420px] lg:h-[560px] rounded-[28px] bg-gradient-to-br from-[#D4A07A] to-[#C08A65] flex items-center justify-center border border-edge">
            <span className="font-fraunces italic text-white/30 text-[15px]">Portrait — Tooba Arsal</span>
          </motion.div>

          <div className="flex flex-col">
            <motion.p {...fadeUp(0)} className="text-[11px] font-semibold tracking-[2.5px] uppercase text-caramel mb-3">How It Started</motion.p>
            <motion.h2 {...fadeUp(0.1)} className="font-fraunces text-[28px] sm:text-[36px] lg:text-[46px] font-normal text-chocolate tracking-[-1.2px] leading-[1.1] mb-6">
            ...
            <motion.p {...fadeUp(0.26)} className="text-base text-muted leading-[1.7] mb-5">
              What started as sharing baked goods with friends and family quickly became something more. People kept asking: &ldquo;Where can we order these?&rdquo; And so, The Cozy Crumbs was born.
            </motion.p>
            ...
            <motion.div {...fadeUp(0.42)} className="mt-9 pt-7 border-t border-edge">
              ...
              <span className="text-xs text-muted font-medium font-inter mt-1 block">Founder & Head Baker, The Cozy Crumbs</span>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-cream py-16 lg:py-24 px-4 sm:px-8 lg:px-20">
        <div className="text-center mb-16">
          ...
          <h2 className="font-fraunces text-[28px] sm:text-[38px] lg:text-[46px] font-normal text-chocolate tracking-[-1.2px] leading-[1.1]">Our Values</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          {VALUES.map((v, i) => (
            <motion.div key={v.title} {...fadeUp(i * 0.1)} className="bg-white rounded-2xl border border-edge p-8 lg:p-10">
      ...
      <section className="bg-chocolate py-16 lg:py-20 px-4 sm:px-8 lg:px-20">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 text-center">
          {STATS.map((s) => (
            <div key={s.label}>
              <p className="font-fraunces text-[36px] sm:text-[42px] lg:text-[52px] text-white tracking-[-1.5px] leading-none mb-2">{s.value}</p>
      ...
      <section className="bg-beige py-16 lg:py-20 px-4 sm:px-8 lg:px-20 text-center">
        <h2 className="font-fraunces text-[28px] sm:text-[36px] lg:text-[42px] font-normal text-chocolate tracking-[-1.2px] mb-4">Ready to Order?</h2>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link href="/shop" className="bg-terracotta text-white px-8 py-4 rounded-full font-medium hover:bg-terracotta/90 transition-colors text-sm">Shop Now</Link>
          <Link href="/contact" className="border border-edge text-ink px-8 py-4 rounded-full font-medium hover:bg-white transition-colors text-sm">Get in Touch</Link>
        </div>
      </section>
    </>
  )
