  ...
  return (
    <>
      <section className="bg-gradient-to-br from-[#F2E4D5] via-[#EDD8C2] to-[#E4C9AA] py-20 lg:py-28 px-4 sm:px-8 lg:px-20 text-center">
        ...
        <h1 className="font-fraunces text-[36px] sm:text-[50px] lg:text-[62px] font-normal text-chocolate tracking-[-1.8px] leading-[1.08] mb-6">Delivery Policy</h1>
      ...
      <section className="bg-cream py-16 lg:py-24 px-4 sm:px-8 lg:px-20">
        <div className="max-w-[960px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          <div className="flex flex-col gap-10">
            <div>
              ...
              <p className="text-sm text-muted leading-[1.7] mb-3">We deliver 7 days a week between <strong className="text-ink">10:00 AM – 8:00 PM</strong>.</p>
              <p className="text-sm text-muted leading-[1.7]">Your preferred delivery time can be requested at checkout. We&apos;ll confirm the exact slot via WhatsApp.</p>
            </div>
            ...
            <div>
              ...
              <p className="text-sm text-muted leading-[1.7] mb-3">All orders are packed in premium, food-safe packaging designed to keep your treats fresh from our kitchen to your door.</p>
              <p className="text-sm text-muted leading-[1.7]">Custom cakes are transported in specially designed boxes to protect their shape. We take extra care with tiered and sculpted cakes.</p>
            </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl border border-edge overflow-hidden">
              {DELIVERY_AREAS.map((row, i) => (
                <div key={row.area} className={`flex justify-between items-center px-6 py-4 text-sm ${i !== DELIVERY_AREAS.length - 1 ? 'border-b border-edge' : ''}`}>
                  <span className="text-ink font-medium">{row.area}</span>
                  <span className="text-muted">{row.charge}</span>
                </div>
              ))}
            </div>
            <p className="text-xs text-muted mt-4 leading-[1.6]">For areas not listed above, message us on WhatsApp at +92 335 0253548.</p>
          </div>
        </div>
      </section>
    </>
