...
export default async function AdminCollectionsPage() {
  ...
  const { data: collections } = await supabase
    ...
    .select('id, name, slug, description, banner_image, is_active, start_date, end_date, created_at')
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-8">
        ...
        <form action={createCollection} className="flex flex-col gap-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Collection Name <span className="text-red-500">*</span></label>
              <input
                name="name" required
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input type="date" name="start_date" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm font-inter focus:outline-none focus:ring-2 focus:ring-caramel/30" />
            </div>
            <div>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      {c.banner_image && (
                        ...
                        <img src={c.banner_image} alt="" className="w-10 h-10 rounded-lg object-cover bg-gray-100" />
                      )}
                      <div>
                        <p className="font-medium text-gray-900">{c.name}</p>
                        {c.description && <p className="text-xs text-gray-400 truncate max-w-[200px]">{c.description}</p>}
                        <p className="text-xs text-gray-300 font-mono">{c.slug}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3 text-xs text-gray-500">
                    {c.start_date ? new Date(c.start_date).toLocaleDateString('en-PK', { day: 'numeric', month: 'short' }) : '—'}
                    {' → '}
                    {c.end_date ? new Date(c.end_date).toLocaleDateString('en-PK', { day: 'numeric', month: 'short' }) : '—'}
                  </td>
                  <td className="px-5 py-3">
