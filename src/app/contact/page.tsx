...
const CONTACT_METHODS = [
  ...
  { icon: Camera, label: 'Instagram', value: '@thecozycrumb180', description: 'See our latest bakes, behind-the-scenes, and daily specials. DMs are welcome.', href: 'https://www.instagram.com/thecozycrumb180/', cta: 'Follow on Instagram', cardClass: 'bg-[#E1306C]/5 border-[#E1306C]/20', iconClass: 'bg-[#E1306C]/10 text-[#E1306C]' },
  { icon: Mail, label: 'Email', value: 'hello@thecozycrumbs.com', description: 'For partnership inquiries, bulk orders, or anything formal.', href: 'mailto:hello@thecozycrumbs.com', cta: 'Send an Email', cardClass: 'bg-caramel/5 border-caramel/20', iconClass: 'bg-caramel/10 text-caramel' },
]
  ...
  return (
    <>
      <section className="bg-gradient-to-br from-[#F2E4D5] via-[#EDD8C2] to-[#E4C9AA] py-20 lg:py-28 px-4 sm:px-8 lg:px-20 text-center">
        ...
        <h1 className="font-fraunces text-[36px] sm:text-[50px] lg:text-[62px] font-normal text-chocolate tracking-[-1.8px] leading-[1.08] mb-6">Get In Touch</h1>
        ...
        <div className="max-w-[960px] mx-auto grid grid-cols-1 sm:grid-cols-3 gap-6">
          {CONTACT_METHODS.map((m) => (
            <div key={m.label} className={`rounded-2xl border p-8 flex flex-col ${m.cardClass}`}>
              ...
              <a href={m.href} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-ink border border-edge bg-white rounded-full px-5 py-2.5 hover:bg-beige transition-colors text-center">
                {m.cta}
              </a>
            </div>
          ))}
      ...
      <section className="bg-beige py-16 lg:py-20 px-4 sm:px-8 lg:px-20">
        <div className="max-w-[960px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16">
          <div>
            ...
            <p className="text-xs text-muted mt-4 leading-[1.6]">WhatsApp messages are typically answered within 1–2 hours during these hours.</p>
          </div>
          ...
          <div>
            ...
            <p className="text-base text-muted leading-[1.7] mb-4">We&apos;re a home bakery based in Karachi, Pakistan. We deliver across all major areas of the city.</p>
            <p className="text-base text-muted leading-[1.7] mb-6">We do not have a physical storefront — all orders are placed online and delivered to your door.</p>
            <Link href="/delivery-policy" className="text-sm font-medium text-caramel hover:text-chocolate transition-colors">View Delivery Areas →</Link>
          </div>
        </div>
      </section>
    </>
