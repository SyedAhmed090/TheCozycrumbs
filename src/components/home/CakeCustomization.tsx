    ...
    <section className="bg-ivory">
      <div className="grid grid-cols-2 gap-20 items-center py-24 px-20">
        <div className="flex flex-col">
          ...
          <motion.h2
            {...fadeUp(0.1)}
            className="font-fraunces text-[46px] font-normal text-chocolate tracking-[-1.2px] leading-[1.1] mb-5"
          >
          ...
          <motion.ul {...fadeUp(0.26)} className="flex flex-col gap-3 mb-9">
            {steps.map((step) => (
              <li
                key={step.name}
                className="flex items-center gap-4 p-4 bg-cream rounded-2xl border border-edge cursor-pointer hover:border-caramel hover:bg-beige transition-all"
              >
          ...
          <motion.div {...fadeUp(0.34)}>
            <button className="bg-chocolate text-white rounded-full px-8 py-4 font-semibold text-sm hover:bg-chocolate-dark transition-all hover:-translate-y-0.5 hover:shadow-lg">
              Design Your Cake
            </button>
          </motion.div>
        </div>
        ...
        <motion.div
          {...fadeUp(0.2)}
          className="h-[480px] rounded-[28px] bg-gradient-to-br from-beige to-[#DEC5A8] border border-edge flex items-center justify-center"
        >
          <span className="font-fraunces italic text-chocolate/30 text-[15px]">
